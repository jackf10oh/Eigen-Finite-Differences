...
struct BumpFunc
{
    ...
    // Member Funcs 
    fdm::Scalar operator()(fdm::Scalar x) const 
    {
      ...
      else if(x < c)
      {
        ...
        fdm::Scalar a = 1 / ((L-c)*(L-c)*(L-c)); 
        fdm::Scalar b = 1 / ((L-c)*(c-L)*(c-L)); 
        fdm::Scalar prefocus = 1 + (x-c)/(c-L) + a*(x-c)*(x-c)*(x-L) + b*(x-c)*(x-L)*(x-L); 
        return h * std::pow(prefocus, focus); 
      }
      else if(x <= R)
      {
        fdm::Scalar a = 1 / ((R-c)*(R-c)*(R-c)); 
        fdm::Scalar b = 1 / ((R-c)*(c-R)*(c-R)); 
        fdm::Scalar prefocus = 1 + (x-c)/(c-R) + a*(x-c)*(x-c)*(x-R) + b*(x-c)*(x-R)*(x-R); 
        return h * std::pow(prefocus, focus); 
      }
      else
      {
